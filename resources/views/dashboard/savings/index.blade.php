    ...
    <div class="row shadow rounded mb-2 mx-0">
        <div class="col-md-12 d-flex justify-content-between align-items-center p-1">
            <ul class="nav nav-pills" id="pills-tab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="btn nav-link" id="pills-exchanges-tab" data-toggle="pill" data-target=".pills-exchanges" type="button" role="tab" aria-controls="pills-exchanges" aria-selected="false">صرف إدخارات</button>
                </li>
            </ul>
            @push('scripts')
                <script>
                ...
                <div class="tab-content" id="pills-tabContent">
                    <div class="tab-pane fade show active pills-table" id="pills-table" role="tabpanel" aria-labelledby="pills-table-tab">
                        <div class="card-body table-container p-0">
                            <table id="savings-table" class="table table-striped table-bordered table-hover sticky" style="width:100%; height: calc(100vh - 130px);">
                                <thead>
                                    <tr>
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#savings-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ...
                    columns: [
                        ...
                        { data: 'total_savings', name: 'total_savings', orderable: false, render: function(data, type, row) {
                            let field = `<x-form.input name="total_savings_${row.id}" style="width: 80px; padding: 0 5px;" type="number" value="${data}" step="0.01" min="0" />`;
                            return field;
                        }},
                        { data: 'edit', name: 'edit', orderable: false, searchable: false, render: function(data, type, row) {
                            @can('edit','App\\Models\ReceivablesLoans')
                            let link = `<button class="btn btn-sm btn-icon text-primary edit_row"  style="padding: 1px;" data-id=":saving"><i class="fe fe-edit"></i></button>`.replace(':saving', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                        ...
                        { data: 'print', name: 'print', orderable: false, searchable: false, render: function (data, type, row) {
                            ...
                            return `
                                <form method="post" action="{{ route('savings.view_pdf', ':savings' )}}" target="_blank">
                                    @csrf
                                    <button
                                        type="submit"
                    ...
                    drawCallback: function(settings) {
                        ...
                        $('#savings-table tbody tr').each(function() {
                ...
                function editReceivablesForm(id) {
                    $.ajax({
                        url: "{{ route('savings.update', ':id') }}".replace(':id', id),
                        method: 'PUT',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        },
                        data: {
                            total_savings: $('input[name="total_savings_' + id + '"]').val(),
        ...
        <script>
            $(document).ready(function() {
                ...
                const rows = $('#savings-table tbody tr');
                ...
                $(document).on('keydown', function(e) {
                    ...
                    const totalRows = $('#savings-table tbody tr:visible').length;
                    const totalCols = $('#savings-table tbody tr:visible').eq(0).find('td').length;
                ...
                $('#savings-table tbody').on('dblclick', 'td', function() {
                    const cell = $(this);
                ...
                function highlightCell(row, col) {
                    ...
                    const visibleRows = $('#savings-table tbody tr:visible');
                    // التحقق من وجود الصف
                    if (visibleRows.length > row) {
                        ...
                        if (targetCell.length) {
                            ...
                            $('#savings-table tbody td').removeClass('active');
